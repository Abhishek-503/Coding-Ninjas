...
public class PathSumRootToLeaf {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		PathSumRootToLeafSol.rootToLeafPathsSumToK(root, k);
	}
}
class PathSumRootToLeafSol {
    ...
    private static void helper(BinaryTreeNode<Integer> root, int k, int sum, String result) {
		//Your code goes here
        if(root==null){
            return;
        }
        if(k==sum+root.data && root.left==null && root.right==null){
            System.out.println(result+root.data);
            return;
        }
        helper(root.left, k, (sum+root.data), (result+root.data+" "));
        
        helper(root.right, k, (sum+root.data), (result+root.data+" "));
	}
